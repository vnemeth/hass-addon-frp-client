...
sed -i "s/serverAddr = \"your_server_addr\"/serverAddr = \"$(bashio::config 'serverAddr')\"/" $CONFIG_PATH
sed -i "s/remotePort = 8123/your_remote_port = $(bashio::config 'remotePort')/" $CONFIG_PATH
sed -i "s/transport.tls.serverName = \"your_server_name\"/transport.tls.serverName = \"$(bashio::config 'serverAddr')\"/" $CONFIG_PATH
